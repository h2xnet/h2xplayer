...
void VideoPlayer::setError(int code, QString msg) {
...
// OnParseParams : 解析参数
//
bool VideoPlayer::OnParseParams() {
    // 判断参数是否为空
    if (params_.isEmpty()) {
...
bool VideoPlayer::OnInputStreamValid() {
    if (!inputAvFmtCtx_) {
        return false;
    }
...
inline bool VideoPlayer::isInited() {
    return inited_;
}

inline bool VideoPlayer::isRunning() {
    return running_;
}

inline bool VideoPlayer::isPause() {
    return pauseing_;
}
...
bool VideoPlayer::getVideoInfo(VideoInfo& info) {
    if (!inputAvFmtCtx_) {
        return false;
    }
